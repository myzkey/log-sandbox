...
export function SlowEndpointsChart({ data }: { data: SlowEndpoint[] }) {
  const chartData = data.map((endpoint) => ({
    name: endpoint.path.length > 30 ? `${endpoint.path.substring(0, 30)}...` : endpoint.path,
  ...
  return (
    <div className="bg-white rounded-lg shadow p-6">
      <h2 className="text-xl font-semibold mb-4 text-gray-900">Top 15 Slowest Endpoints</h2>
