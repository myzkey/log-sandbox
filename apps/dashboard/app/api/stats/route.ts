...
export async function GET(request: NextRequest) {
  ...
  const avgResponseTime = await (whereClause
    ? db.select({ avg: sql<number>`avg(${albLogs.totalTime})` }).from(albLogs).where(whereClause)
  ...
  const clientErrorCount = await (whereClause
    ? db.select({ count: sql<number>`count(*)` }).from(albLogs).where(and(whereClause, clientErrorCondition)!)
  ...
  const serverErrorCount = await (whereClause
    ? db.select({ count: sql<number>`count(*)` }).from(albLogs).where(and(whereClause, serverErrorCondition)!)
  ...
  const timeoutCount = await (whereClause
    ? db.select({ count: sql<number>`count(*)` }).from(albLogs).where(and(whereClause, timeoutCondition)!)
