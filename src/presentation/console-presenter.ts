...
export class ConsolePresenter {
  present(result: AnalysisResult, options: PresentationOptions = {}): void {
    ...
    const slowRequestLimit =
      options.slowRequestLimit !== undefined ? options.slowRequestLimit : 100;
    ...
    if (result.timeouts.length > 0) {
      console.log(
        `タイムアウト: ${result.timeouts.length} (${(
          (result.timeouts.length / result.entries.length) *
          100
        ).toFixed(1)}%)`
      );
    }
    if (result.rejectedRequests.length > 0) {
      console.log(
        `拒否されたリクエスト: ${result.rejectedRequests.length} (${(
          (result.rejectedRequests.length / result.entries.length) *
          100
        ).toFixed(1)}%)`
      );
    }
    ...
    if (result.responseTimes.length > 0 && result.stats) {
      ...
      console.log(
        `  分析対象リクエスト: ${result.responseTimes.length} (タイムアウト・拒否を除く)`
      );
    ...
    for (const [code, count] of sortedStatusCodes) {
      ...
      console.log(
        `  ${code}: ${count.toString().padStart(6)} (${percentage
          .toFixed(1)
          .padStart(5)}%)`
      );
    }
    ...
    for (const [method, count] of sortedMethods) {
      const percentage = (count / result.entries.length) * 100;
      console.log(
        `  ${method}: ${count.toString().padStart(6)} (${percentage
          .toFixed(1)
          .padStart(5)}%)`
      );
    }
    ...
    for (const [ip, count] of topIPs) {
      const percentage = (count / result.entries.length) * 100;
      console.log(
        `  ${ip}: ${count.toString().padStart(6)} (${percentage
          .toFixed(1)
          .padStart(5)}%)`
      );
    }
    ...
    if (result.timeouts.length > 0) {
      ...
      for (let i = 0; i < Math.min(10, result.timeouts.length); i++) {
        ...
        console.log(
          `  ${i + 1}. [${entry.timestamp}] ${entry.elbStatusCode} - ${
            entry.requestMethod
          } ${entry.requestPath}`
        );
    ...
    if (result.rejectedRequests.length > 0) {
      console.log(
        `拒否されたリクエスト（ALBレベル）: ${result.rejectedRequests.length}`
      );
      console.log("\n拒否リクエスト詳細:");
    ...
    if (result.errors.length > 0) {
      ...
      for (let i = 0; i < Math.min(10, result.errors.length); i++) {
        ...
        console.log(
          `  ${i + 1}. [${entry.timestamp}] ${entry.targetStatusCode} - ${
            entry.requestMethod
          } ${entry.requestPath}`
        );
        console.log(`     クライアント: ${entry.clientIp}`);
    ...
    // Slow requests
    const slowRequests = result.entries.filter(
      (e) => e.totalTime > slowRequestThreshold
    );
    if (slowRequests.length > 0) {
      const sorted = [...slowRequests].sort(
        (a, b) => b.totalTime - a.totalTime
      );
      const displayLimit = slowRequestLimit
        ? Math.min(slowRequestLimit, sorted.length)
        : sorted.length;
      ...
      for (let i = 0; i < displayLimit; i++) {
        ...
        console.log(
          `   メソッド:    ${entry.requestMethod} ${entry.requestPath}`
        );
    ...
    if (result.timeAnalysis.length > 0) {
      ...
      for (const bucket of result.timeAnalysis) {
        ...
        const avg =
          bucket.avgResponseTime > 0
            ? bucket.avgResponseTime.toFixed(3).padStart(6)
            : "  N/A";
        const max =
          bucket.maxResponseTime > 0
            ? bucket.maxResponseTime.toFixed(3).padStart(6)
            : "  N/A";
        const errors = bucket.errors.toString().padStart(6);
        const timeouts = bucket.timeouts.toString().padStart(10);

        console.log(
          `${time}    | ${count} | ${avg} | ${max} | ${errors} | ${timeouts}`
        );
      }

      console.log();

      const sortedByCount = [...result.timeAnalysis].sort(
        (a, b) => b.count - a.count
      );
      const sortedByAvgTime = [...result.timeAnalysis].sort(
      ...
      for (let i = 0; i < Math.min(3, sortedByCount.length); i++) {
        ...
        console.log(
          `  ${i + 1}. ${bucket.timestamp.substring(11)} - ${
            bucket.count
          } リクエスト`
        );
      }
      ...
      for (let i = 0; i < Math.min(3, sortedByAvgTime.length); i++) {
        ...
        if (bucket.avgResponseTime > 0) {
          console.log(
            `  ${i + 1}. ${bucket.timestamp.substring(
              11
            )} - ${bucket.avgResponseTime.toFixed(3)}s 平均 (${
              bucket.count
            } リクエスト)`
          );
        }
