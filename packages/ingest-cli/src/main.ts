...
async function main(): Promise<void> {
  ...
  let slowRequestLimit: number | null | undefined
  ...
  for (const arg of args) {
    if (arg.startsWith('--output=')) {
      ...
      slowRequestLimit = value === 'all' ? null : parseInt(value, 10)
  ...
  try {
    ...
    const logReader = filePath ? new FileLogReader(filePath) : new StdinLogReader()
