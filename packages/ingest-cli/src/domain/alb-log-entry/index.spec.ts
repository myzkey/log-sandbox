...
describe('ALBLogEntry', () => {
  ...
  describe('parse', () => {
    it('正常なログ行をパースできること', () => {
      ...
      expect(entry.responseProcessingTime).toBe(0.0)
      expect(entry.elbStatusCode).toBe('204')
      expect(entry.targetStatusCode).toBe('204')
      expect(entry.requestMethod).toBe('OPTIONS')
      expect(entry.requestUrl).toBe(
        'https://api.example.com:443/v1/services/123/items/456/location',
      )
    ...
    it('タイムアウトを正しく判定すること', () => {
      const timeoutLog =
        'h2 2025-10-28T01:41:11.673240Z app/my-alb/abc123 203.0.113.10:40742 10.0.1.100:3000 -1 -1 -1 504 504 58 231 "GET https://api.example.com:443/test HTTP/2.0" "Mozilla/5.0" - - - - - - - - - - - - - -'
  ...
  describe('エッジケース', () => {
    it('クエリパラメータ付きのURLを正しくパースすること', () => {
      const logWithQuery =
        'h2 2025-10-28T01:41:11.673240Z app/my-alb/abc123 203.0.113.10:40742 10.0.1.100:3000 0.002 0.526 0.000 200 200 58 231 "GET https://api.example.com:443/test?foo=bar&baz=qux HTTP/2.0" "Mozilla/5.0" - - - - - - - - - - - - - -'
    ...
    it('ポート番号なしのURLを正しくパースすること', () => {
      const logWithoutPort =
        'h2 2025-10-28T01:41:11.673240Z app/my-alb/abc123 203.0.113.10:40742 10.0.1.100:3000 0.002 0.526 0.000 200 200 58 231 "GET https://api.example.com/test HTTP/2.0" "Mozilla/5.0" - - - - - - - - - - - - - -'
