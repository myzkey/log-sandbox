...
export class ConsolePresenter {
  present(result: AnalysisResult, options: PresentationOptions = {}): void {
    ...
    if (result.timeouts.length > 0) {
      console.log(
        `タイムアウト: ${result.timeouts.length} (${(
          (result.timeouts.length / result.entries.length) * 100
        ).toFixed(1)}%)`,
      )
    }
    if (result.rejectedRequests.length > 0) {
      console.log(
        `拒否されたリクエスト: ${result.rejectedRequests.length} (${(
          (result.rejectedRequests.length / result.entries.length) * 100
        ).toFixed(1)}%)`,
      )
    }
    console.log()
    ...
    for (const [code, count] of sortedStatusCodes) {
      ...
      console.log(
        `  ${code}: ${count.toString().padStart(6)} (${percentage.toFixed(1).padStart(5)}%)`,
      )
    }
    console.log()
    ...
    for (const [method, count] of sortedMethods) {
      const percentage = (count / result.entries.length) * 100
      console.log(
        `  ${method}: ${count.toString().padStart(6)} (${percentage.toFixed(1).padStart(5)}%)`,
      )
    }
    console.log()
    ...
    for (const [ip, count] of topIPs) {
      const percentage = (count / result.entries.length) * 100
      console.log(
        `  ${ip}: ${count.toString().padStart(6)} (${percentage.toFixed(1).padStart(5)}%)`,
      )
    }
    console.log()
    ...
    if (result.timeAnalysis.length > 0) {
      ...
      for (const bucket of result.timeAnalysis) {
        ...
        const avg =
          bucket.avgResponseTime > 0 ? bucket.avgResponseTime.toFixed(3).padStart(6) : '  N/A'
        const max =
          bucket.maxResponseTime > 0 ? bucket.maxResponseTime.toFixed(3).padStart(6) : '  N/A'
        ...
        if (bucket.avgResponseTime > 0) {
          console.log(
            `  ${i + 1}. ${bucket.timestamp.substring(
              11,
            )} - ${bucket.avgResponseTime.toFixed(3)}s 平均 (${bucket.count} リクエスト)`,
          )
        }
      }
      console.log()
